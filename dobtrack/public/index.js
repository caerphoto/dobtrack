...
DTR.updateURLs = function() {
    ...
    $("#lnkExport").attr("href", "/dobitems.csv" + DTR.urlParams());
...
DTR.getItems = function() {
    ...
    $.getJSON("/getitems" + DTR.urlParams(), function(data) {
        ...
        if (!DTR.username) {
            DTR.buildCustomerList(data.username);
        }
...
DTR.buildCustomerList = function(username) {
...
DTR.init = function() {
    ...
    // Hide the "Copy to clipboard" button(image) if not using IE
